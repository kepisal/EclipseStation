...
-- ----------------------------
BEGIN;
	INSERT INTO tbuser(username, email, password) VALUES('admin','admin@hrd.com', '123');
COMMIT;
...
ALTER FUNCTION search_by_title(text, integer, integer)
  OWNER TO postgres;
---------------------------------------------------------------------------------------
-- Function: sort_author(integer, boolean)

-- DROP FUNCTION sort_author(integer, boolean);

CREATE OR REPLACE FUNCTION sort_by_author(
    r integer,
    of integer,
    isasc boolean)
  RETURNS SETOF tbarticle AS
...
IF isAsc = TRUE THEN 
RETURN QUERY 
	SELECT * FROM tbarticle ORDER BY author ASC LIMIT r OFFSET of; 
ELSE 
RETURN QUERY 
	SELECT * FROM tbarticle ORDER BY author  DESC LIMIT r OFFSET of; 
END IF; 
END $BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100
  ROWS 1000;
ALTER FUNCTION sort_by_author(integer, integer, boolean)
  OWNER TO postgres;
-------------------------------------------------------------------------------------------
-- Function: sort_id(integer, boolean)

-- DROP FUNCTION sort_id(integer, boolean);
CREATE OR REPLACE FUNCTION sort_id(
    r integer,
    of integer,
    isasc boolean)
  RETURNS SETOF tbarticle AS
...
IF isAsc = TRUE THEN
	RETURN QUERY 
	SELECT * FROM tbarticle ORDER BY id ASC LIMIT r OFFSET of; 
ELSE 
	RETURN QUERY
	SELECT * FROM tbarticle ORDER BY id DESC LIMIT r OFFSET of;  
END IF; 
END $BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100
  ROWS 1000;
ALTER FUNCTION sort_id(integer, integer, boolean)
  OWNER TO postgres;
...
CREATE OR REPLACE FUNCTION sort_title(
    r integer,
    of integer,
    isasc boolean)
  RETURNS SETOF tbarticle AS
...
IF isAsc = TRUE THEN 
RETURN QUERY 
	SELECT * FROM tbarticle ORDER BY title ASC LIMIT r OFFSET of; 
ELSE 
RETURN QUERY 
	SELECT * FROM tbarticle ORDER BY title DESC LIMIT r OFFSET of;
END IF; 
END $BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100
  ROWS 1000;
ALTER FUNCTION sort_title(integer, integer, boolean)
  OWNER TO postgres;
